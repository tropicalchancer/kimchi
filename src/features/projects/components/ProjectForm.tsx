...
export function ProjectForm({ isOpen, onClose, onProjectCreated, project }: ProjectFormProps) {
  ...
  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const currentSlugIsAutoGenerated = !slug || slug === newTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')
        ...
        <div className="mt-6 flex justify-between items-center">
          {project && (
            <button
              ...
              className="px-4 py-2 text-base text-red-600 hover:text-red-700 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isDeleting}
            >
